...
void directInsert()
{
    ...
    for(i=1; i<=10; i++)
    {
        ...
        sprintf(sDstIP, "2001:0DB8:0000:0000:0000:0000:1428:%04d", i);
        sprintf(sRegDate, "2015-03-31 15:26:%02d", i);
        sprintf(sLog, "text log-%d", i);
        sprintf(sImage, "binary image-%d", i);

        memset(query, 0x00, sizeof(query));

        if( i == 10 )
        {
            sprintf(query, "INSERT INTO CLI_SAMPLE1 VALUES(NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)");
        }
        else
        {
            sprintf(query, "INSERT INTO CLI_SAMPLE1 VALUES(%d, %d, %ld, %f, %f, '%s', '%s', '%s',TO_DATE('%s','YYYY-MM-DD HH24:MI:SS'),'%s','%s')", sSeq, sScore, sTotal, sPercentage, sRatio, sId, sSrcIP, sDstIP, sRegDate, sLog, sImage);
        }
